...
ggplot(
    ...
    geom_histogram() +
    geom_vline(color = "red", xintercept = 1.00) +
...
gene_values <- df_khan_2[, 11] %>% pull()
shapiro.test(gene_values)
```
...
>Q8. Reqrite the code so it tests group 2 and group 4 and do it for all genes.
