...
(totalx2 <- x2abstainers + x2drinkers)
...
for (i in seq_len(length(x))) {
    # Get data
    data <- snpdata$contingency_table[[i]]
    # Calculate stuff
    x[i] <- chisq.test(data, correct = T)$p.value
}
...
snpdata %>%
    filter(p.value <= (0.05 / nrow(snpdata))) %>%
    count()
```
...
snpdata %>%
    group_by(chromosome) %>%
    summarise(sig = sum(p.value <= 0.005), sigpercent = sum(p.value <= 0.005) / n()) %>%
