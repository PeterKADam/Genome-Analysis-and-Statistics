...
1 is slightly more probable with lambda1
...
exptected <- tibble(
    ...
    probability = dpois(x = xs, lambda = mean(extinctData$numberOfExtinctions)) * 76
)
```
...
```{r}
(pooled_exp <- c(sum(exptected$probability[1:2]), exptected$probability[3:8], sum(exptected$probability[9:21])))
(pooled_obs <- c(sum(extinctTable[1:2]), extinctTable[3:8], sum(extinctTable[9:21])))
```
...
```{r}
(x2 <- (chisq.test(pooled_obs, p = pooled_exp / sum(pooled_exp))$statistic))
```
...
```{r}
(df_nematode <- tibble(nr_nematodes = 0:6, nr_fish = c(103, 72, 44, 14, 3, 1, 1)))
```


```{r}
df_nematode %>% ggplot(aes(x = nr_nematodes, y = log(nr_fish, base = 10))) +
