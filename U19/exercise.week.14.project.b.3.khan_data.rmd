...
```{r}
p1 <- results %>% ggplot(aes(x = log_fc, y = -log(pval_wilcox, base = 10))) +
    geom_point()
p2 <- results %>% ggplot(aes(x = log_fc, y = -log(pval_wilcox_b, base = 10))) +
    geom_point()
p3 <- results %>% ggplot(aes(x = log_fc, y = -log(pval_wilcox_fdr, base = 10))) +
    geom_point()

results %>% plot_grid(p1, p2, p3, nrow = 1, ncol = 4)
```
...
cat("same variance t-test power: ", ((sum(p < 0.05) / length(p))), "\n")
cat("same variance welch t-test power: ", ((sum(p2 < 0.05) / length(p2))), "\n")
cat("same variance wilcox test power: ", ((sum(p3 < 0.05) / length(p3))), "\n")
```
...
cat("different variance t-test power: ", ((sum(p < 0.05) / length(p))), "\n")
cat("different variance welch t-test power: ", ((sum(p2 < 0.05) / length(p2))), "\n")
cat("different variance wilcox test power: ", ((sum(p3 < 0.05) / length(p3))), "\n")
```
...
for (i in 1:length(p)) {
    g1 <- runif(n = 25, min = 50, max = 150)
    g2 <- 5 + runif(n = 29, min = 50, max = 150)

    p[i] <- t.test(g1, g2, var.equal = T)$p.value
    p2[i] <- t.test(g1, g2)$p.value
    p3[i] <- wilcox.test(g1, g2)$p.value
}
cat("not normal t-test power: ", ((sum(p < 0.05) / length(p))), "\n")
cat("not normal welch t-test power: ", ((sum(p2 < 0.05) / length(p2))), "\n")
cat("not normal wilcox test power: ", ((sum(p3 < 0.05) / length(p3))), "\n")
```
