...
df %>% filter(life_expec_2002 > life_expec_1992) %>% nrow()
...
df %>%
  group_by(continent) %>% 
  summarise(score_higher = sum( life_expec_2002> 60),
            n_total = n(),
            proportion = score_higher/n_total)

(df2 <- df %>% filter(continent=="Asia") )
df2 %>% filter(life_expec_2002 > 60) %>% nrow()
df2 %>% nrow()
...
df %>% 
  ...
  group_by(continent) %>%
  summarise(up = sum(life_expec_2002 > life_expec_1992),
            n = n(),
            p = up/n) %>%
  arrange(desc(p))
