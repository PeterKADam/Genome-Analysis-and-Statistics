...
df %>% filter(size==40 & effect==0.96)

(result <- df %>% filter(size==40 & effect==0.96) %>% nrow())
...
So we should ONLY USE the simulations for effect size = 1 (where drug = placebo)

Then we will calculate the proportion of these that are significant. 

For all other simulation we know that H0 is false (i.e. drug != placebo).

```{r}

df2 <- df %>% filter(effect==1.0)

sum(df2$p <= 0.05) # significant = TRUE positives
sum(df2$p > 0.05) # not significant = FALSE NEGATIVES
nrow(df2) # All the positives
...
nrow(df2) # they should all be significant since effect = 0.98
sum(df2$p <= 0.05) # significant ones = true positives
sum(df2$p > 0.05)  # not significant = false negatives
sum(df2$p <= 0.05) / nrow(df2)  # power
1 - (sum(df2$p > 0.05) / nrow(df2)) # power (same result)
(result <- sum(df2$p <= 0.05) / nrow(df2))
